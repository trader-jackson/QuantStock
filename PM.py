...
class PM(ABC):  # Inherit from ABC to use abstractmethod
    def __init__(self, args, data_all):
        ...
        if not hasattr(args, 'log_dir') or args.log_dir is None:
            # Create log directory based on project name and model
            if hasattr(args, 'project_name') and hasattr(args, 'model'):
                log_dir = os.path.join('log', 'pred_' + args.project_name + "_" + args.model)
            else:
                log_dir = os.path.join('log', 'pred_default')
        else:
    ...
    def _get_data(self, flag):
        ...
        dataset = DatasetStock_PRED(self.data_all, type=flag)
